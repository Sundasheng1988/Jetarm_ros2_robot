...
> 最后更新：2026-07-04 | 当前阶段：Mobile Robot Foundation（移动机器人阶段） | 当前目标：AMCL → Nav2 → MoveSkill
...
| `/tf_static`     | 静态 TF                        |
| `/cmd_vel`       | 底盘速度控制接口                     |
| `/mobile_base/sensors/imu_data` | IMU 数据（当前未接入 AMCL/Nav2） |
...
| `/goal_pose`      | RViz Goal Pose → Nav2 导航目标（未验证）   |
